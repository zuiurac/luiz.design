<div class="qode-info-card-with-icon">
    <div class="qode-icwi-image-icon-holder qode-start-icon-hover">
		...
		<div class="qode-icwi-icon-holder">
			<span <?php bridge_qode_class_attribute($holder_classes); ?>  <?php echo bridge_qode_get_inline_style($icon_holder_style); ?> <?php echo bridge_qode_get_inline_attrs($icon_holder_data); ?>>
				<?php bridge_qode_icon_collections()->renderIconHTML($icon, $icon_pack, array('icon_attributes' => array('class' => 'qode-icon-element', 'style'=> $icon_style))); ?>
			</span>
		</div>
		<?php if($link != ''): ?>
			</a>
