<div <?php bridge_qode_class_attribute($holder_classes); ?>>
    <div class="qode-giwt-inner <?php echo esc_attr($gradient_classes); ?>">
        <?php if(!empty($link)) : ?>
            <a class="qode-iwt-link" href="<?php echo esc_attr($link); ?>" <?php bridge_qode_inline_attr($target, 'target'); ?>>
        <?php endif; ?>
            <span class="qode-giwt-icon-holder">
                <?php echo bridge_core_get_shortcode_template_part('templates/icon', 'gradient-icon-with-text', '', array('icon_parameters' => $icon_parameters)); ?>
            </span>
        <?php if(!empty($link)) : ?>
            </a>
        <?php endif; ?>
