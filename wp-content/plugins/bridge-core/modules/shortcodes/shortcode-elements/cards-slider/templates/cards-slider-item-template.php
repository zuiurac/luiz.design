...
<div class="pane card<?php echo esc_html($rand_number); ?> <?php echo esc_attr($additional_classes); ?>">
    <?php if($show_card){ ?>
    <div class="card" data-value="card<?php echo esc_html($rand_number); ?>"><div class="card-inner" <?php bridge_qode_inline_style(array($background_color,$header_image)); ?>></div></div>
    <?php } ?>
    <div class="qode-card-slider-holder-outer" <?php bridge_qode_inline_style(array($background_color)); ?>>
            ...
            <div class="qode-card-slider clearfix">
                <?php foreach($images as $i=>$image) { ?>
                    <div class='slide' data-card-slide="<?php echo esc_attr($i+1); ?>">
                        ...
                        <?php } ?>
                            <img src="#" alt="" class="qode-lazy-image" data-image="<?php echo esc_url($image['url']);?>" <?php bridge_qode_inline_style(array('width:'.$image['width'].'px','height:1px')); ?>data-ratio="<?php echo esc_attr($image['height']/$image['width']); ?>" data-lazy="true" />
                        <?php if($image['image_link'] !== ''){ ?>
                            </a>
                        <?php } ?>
                    </div>
